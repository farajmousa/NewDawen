...
import '../../data/model/entity/location_data.dart';

class UserLocationsListBloc extends BaseBloc<Result<List<LocationData>>> {
  ApiRepo repository;

  UserLocationsListBloc(this.repository);
  ...
  Future<void> get() async {
    ...
    try {
      ...
      var result = await repository.call(
        ApiMethod.get,
        "${Urls.employeeLocations}/$userId",
      );

        List<LocationData> dataList = (jsonDecode(result) as List)
      ...
      appLog("Error UserLocationsListBloc: ${e.toString()}");
      emit(Result.error('codeBadRequest'));
    }
  }
}
