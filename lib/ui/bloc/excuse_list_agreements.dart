...
class ExcuseListAgreementBloc extends BaseBloc<Result<List<ExcuseAgreementData>>> {
  ApiRepo repository;

  ExcuseListAgreementBloc(this.repository);
  ...
  Future<void> get() async {
    ...
    try {
      var result = await repository.call(
        ApiMethod.get,
        "${Urls.excuseGetAllAgreements}?Empid=${sm.getUser()?.usId ?? 0}&StartFrom=1&EndTo=1000&lang=$currentLocale",
        addLang: false
      );
      ...
      appLog("Error ExcuseListAgreementBloc: ${e.toString()}");
      emit(Result.error('codeBadRequest'));
    }
  }
}
