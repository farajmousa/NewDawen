...
class ExcuseListBloc extends BaseBloc<Result<List<ExcuseData>>> {
  ApiRepo repository;

  ExcuseListBloc(this.repository);
  ...
  Future<void> get() async {
    ...
    try {
      ...
      var result = await repository.call(
        ApiMethod.get,
        "${Urls.excuseGetAll}/$userId",
      );

        List<ExcuseData> dataList = (jsonDecode(result) as List)
            .map((itemWord) => ExcuseData.fromJson(itemWord))
            .toList();
      ...
      appLog("Error ExcuseListBloc: ${e.toString()}");
      emit(Result.error('codeBadRequest'));
    }
  }
}
